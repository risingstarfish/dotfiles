...
if (-not (Test-Path $installerPath)) {
    ...
    for ($attempt = 1; $attempt -le $MAX_DOWNLOAD_ATTEMPT; $attempt++) {
        try {
            Invoke-WebRequest -Uri $installerUrl -OutFile $installerPath -UseBasicParsing -ErrorAction Stop
            break 
        }
        catch {
            if ($attempt -lt $MAX_DOWNLOAD_ATTEMPT) {
                Write-Host "         -> Attempt $attempt failed. Retrying in 3 seconds... ($_)" -ForegroundColor DarkYellow
                Start-Sleep -Seconds 3
            }
            else {
                Write-Host "         -> Failed to download installer after $MAX_DOWNLOAD_ATTEMPT attempts. $_" -ForegroundColor Red
                exit 1
            }
        }
    }
...
if ($TestRun) {
    ...
    if (Test-Path $installerPath) {
        try {
            ...
            Write-Host "         -> Could not remove temporary installer. $_" -ForegroundColor Yellow
            Write-Host "         -> Manual removal required at: $installerPath" -ForegroundColor Yellow
        }
    }
    else {
        Write-Host "         -> Temporary installer not found (cleaned, skipped download, or error)." -ForegroundColor Yellow
    }
