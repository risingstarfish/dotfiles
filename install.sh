...
ACTIONS
  -u, --update              Update existing configurations only
  -c, --clean               Remove orphaned symlinks, then install/relink
      --reset               Remove all symlinks managed by this tool
  -i, --install <module>    Run only for the specified module(s)
  -x, --exclude <module>    Run for all modules EXCEPT those specified
...
PACKAGES
      --no-packages         Skip dotfiles dependency installation
...
dotfiles::unset_all() {
    unset -f dotfiles::println dotfiles::print_banner dotfiles::print_start dotfiles::print_end \
        dotfiles::print_help dotfiles::print_version \
